...
class Byjunopayment extends \Magento\Payment\Model\Method\Adapter
{
    ...
    public function isTheSame(\ByjunoCheckout\ByjunoCheckoutCore\Helper\Api\ByjunoRequest $request) {
        ...
        if ($this->_scopeConfig->getValue('byjunocheckoutsettings/ByjunoCheckout_setup/cdpbeforeshow', \Magento\Store\Model\ScopeInterface::SCOPE_STORE) == '1'
            ...
            try {
                ...
                if (!$this->isTheSame($request) || empty($CDPStatus)) {
                    ...
                    if ($request->getCompanyName1() != '' && $this->_dataHelper->_scopeConfig->getValue('byjunocheckoutsettings/ByjunoCheckout_setup/businesstobusiness',
                    ...
                    $response = $byjunoCommunicator->sendRequest($xml, (int)$this->_dataHelper->_scopeConfig->getValue('byjunocheckoutsettings/ByjunoCheckout_setup/timeout',
    ...
    public function capture(InfoInterface $payment, $amount)
    {
        ...
        $order = $payment->getOrder();
        $webshopProfileId = $payment->getAdditionalInformation("webshop_profile_id");
        $invoice = \ByjunoCheckout\ByjunoCheckoutCore\Observer\InvoiceObserver::$Invoice;
        if ($invoice == null) {
            throw new LocalizedException(
        ...
        if ($status == 'ERR') {
            throw new LocalizedException(
                __($this->_scopeConfig->getValue('byjunocheckoutsettings/localization/ByjunoCheckout_s4_fail', \Magento\Store\Model\ScopeInterface::SCOPE_STORE, $webshopProfileId). " (error code: CDP_FAIL)")
            );
        } else {
